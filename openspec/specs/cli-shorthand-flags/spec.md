...
The `flacser convert` command SHALL accept documented single-letter shorthand aliases for supported options while preserving the existing long-form flags. The shorthand contract SHALL be `-o` for `--output-dir`, `-w` for `--overwrite`, `-n` for `--dry-run`, `-r` for `--recursive`, `-f` for `--flatten`, and `-j` for `--jobs`.
